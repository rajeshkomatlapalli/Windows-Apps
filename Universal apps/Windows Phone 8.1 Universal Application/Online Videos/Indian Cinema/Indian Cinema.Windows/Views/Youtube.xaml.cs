...
namespace OnlineVideos.Views
{
    ...
    public sealed partial class Youtube : Page
    {
        ...
        OnlineVideos.Views.AppInsights insights = new OnlineVideos.Views.AppInsights();
        ...
        public async void youtub()
        {
            try
            {
                AppSettings.YoutubeUri = string.Empty;
                var url = await MyToolkit.Multimedia.YouTube.GetVideoUriAsync(AppSettings.LinkUrl, MyToolkit.Multimedia.YouTubeQuality.Quality480P);
                Uri url1 = new Uri(url.Uri.AbsoluteUri);
                if (url1 != null)
                {
                    BackButton.Visibility = Visibility.Visible;
                    mediaplayer.Source = url1;
                }
                else
                {
                ...
                //Task.Run(async () => await MyToolkit.Multimedia.YouTube.GetVideoUriAsync(AppSettings.LinkUrl, MyToolkit.Multimedia.YouTubeQuality.Quality480P));
        ...
        public void ChangeWidth()
        {
            try
            {
                IEnumerable<DependencyObject> cboxes = GetChildsRecursive(mediaplayer);
                foreach (DependencyObject obj in cboxes.OfType<Grid>())
                {
                    ...
                    if (type.Name == "Grid" && (obj as Grid).Name == "TimelineContainerGrid")
                    {
                        ...
                        if (adstack.Visibility == Visibility.Visible)
                        {
                            try
                            {
                                mediaplayer.LayoutUpdated -= videoMediaElement_LayoutUpdated;
                            }
                            catch (Exception)
                            {
                            }
        ...
        private void MediaPlayer_MediaOpened_1(object sender, RoutedEventArgs e)
        {
            ...
            //string mve = string.Empty;
            //mve = AppSettings.LinkType + " " + AppSettings.Title;
            mediaplayer.Position = Constants.MediaElementPosition;
            BackButton.Visibility = Visibility.Visible;
            //stopwatch = System.Diagnostics.Stopwatch.StartNew();
            //var properties = new Dictionary<string, string> { { AppSettings.PlayVideoTitle, " View" } };
            //var metrics = new Dictionary<string, double> { { "Duration", stopwatch.Elapsed.Seconds } };
            //insights.Event("Youtube", properties, metrics);
            var success = false;
        ...
        private void MediaPlayer_MediaEnded_1(object sender, MediaPlayerActionEventArgs e)
        {
            try
            {
                stopwatch.Stop();
        ...
        {
            if (mediaplayer.PlaybackRate.ToString() == "1")
            {
            ...
            else
            {
                ...
                if (mediaplayer.PlaybackRate.ToString().Contains("-"))
                {
                    rate.Text = "Rewind " + mediaplayer.PlaybackRate.ToString() + "X";
                }
                else
                {
                    rate.Text = "FastForward " + mediaplayer.PlaybackRate.ToString() + "X";
                }
            }
        }
