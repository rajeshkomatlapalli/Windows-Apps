    ...
    public static class SilverlightSerializer
    {
                    ...
                    if (attribute != null)
                    {
                        ...
                        if (attrs != null)
                        {
                            foreach (var attr in attrs)
                                function(tp.GetType(), attr);
                        }
                    }
                    else
                        function(tp.GetType(), null);
                }
            }
            catch (Exception)
            {
        ...
        private static IEnumerable<PropertyInfo> GetPropertyInfo(Type itm)
        {
            lock (PropertyLists)
            {
                ...
                if (!IsChecksum)
                {
                    ...
                    {
                        ret = itm.GetType().GetTypeInfo().DeclaredProperties.Where(p => p.GetCustomAttributes(typeof(DoNotSerialize), false).Count() == 0 && !(p.GetIndexParameters().Count() > 0) && (p.SetMethod != null)).ToArray();
                ...
                else
                {
                    ...
                    {
                        ret = itm.GetType().GetTypeInfo().DeclaredProperties.Where(p => p.GetCustomAttributes(typeof(DoNotSerialize), false).Count() == 0 && p.GetCustomAttributes(typeof(DoNotChecksum), true).Count() == 0 && !(p.GetIndexParameters().Count() > 0) && (p.SetMethod != null)).ToArray();
        ...
        private static IEnumerable<FieldInfo> GetFieldInfo(Type itm)
        {
            lock (FieldLists)
            {
                ...
                {
                    ret = FieldLists[itm] = itm.GetType().GetTypeInfo().DeclaredFields.Where(p => p.GetCustomAttributes(typeof(DoNotSerialize), false).Count() == 0).ToArray();
                }
        ...
        private static object DeserializeObject(BinaryReader reader, Type itemType = null, object instance = null)
        {
            ...
            if (itemType != null)
            {
                ...
                {
                    if (itemType.GetType().GetTypeInfo().IsEnum)
                    {
        ...
        private static object DeserializeDictionary(IDictionary o, Type itemType, BinaryReader reader)
        {
            Type keyType = null;
            Type valueType = null;
            if (itemType.GetType().GetTypeInfo().IsGenericType)
            {
                var types = itemType.GetType().GetTypeInfo().GenericTypeArguments;
                keyType = types[0];
                valueType = types[1];
            }
        ...
        private static void DeserializeProperties(BinaryReader reader, Type itemType, object o)
        {
            ...
            for (var i = 0; i < propCount; i++)
            {
                ...
                var propType = itemType.GetType().GetTypeInfo().GetDeclaredProperty(propName);
        ...
        private static void DeserializeFields(BinaryReader reader, Type itemType, object o)
        {
            ...
            for (var i = 0; i < fieldCount; i++)
            {
                ...
                var fieldType = itemType.GetType().GetTypeInfo().GetDeclaredField(fieldName);
        ...
        private static void SerializeObject(object item, BinaryWriter writer, Type propertyType = null)
        {
            ...
            if (IsSimpleType(itemType))
            {
                if (itemType.GetType().GetTypeInfo().IsEnum)
                {
                    if (itemType.Name == "Visibility")
                    {
                        int v = (Visibility)item == Visibility.Collapsed ? 1 : 0;
                        WriteValue(writer, v);
                    }
                    else
                    {
        ...
        private static void SerializeList(IList item, Type tp, BinaryWriter writer)
        {
            ...
            if (tp.GetType().GetTypeInfo().IsGenericType)
            {
                var types = tp.GetType().GetTypeInfo().GenericTypeArguments;
                valueType = types[0];
            }
        ...
        {
            return tp.GetType().GetTypeInfo().IsPrimitive || tp == typeof(DateTime) || tp == typeof(TimeSpan) || tp == typeof(string) || tp.GetType().GetTypeInfo().IsEnum || tp == typeof(Guid) || tp == typeof(decimal);
        }
        ...
        private static object CreateObject(Type itemType)
        {
            ...
            {
                return itemType.GetType().GetTypeInfo().DeclaredConstructors.Select(i => i.Invoke(new object[] { }));
