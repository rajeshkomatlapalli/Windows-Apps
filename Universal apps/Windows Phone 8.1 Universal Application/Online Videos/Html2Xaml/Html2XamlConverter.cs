...
namespace Html2Xaml
{
    ...
    public static class Html2XamlConverter
	{
        ...
        static List<string> CustomList = new List<string>() {"Durmuhurtam:", "Yamagandam:","Sun in","Moon in","Rahukalam:" };
        ...
        public static string gettitle(string HtmlString)
        {
            ...
            return doc.DocumentNode.ChildNodes[0].InnerText;
        }
		/// <summary>
		...
		public static string Convert2Xaml(string HtmlString)
		{
            ...
            List<int> customindex = new List<int>();
            int count = 0;
			populateAttributes();
			HtmlDocument doc = new HtmlDocument();
			doc.LoadHtml(HtmlString);
            ...
            foreach (string condition in CustomList)
            {
                IEnumerable<HtmlNode> nodlist = doc.DocumentNode.ChildNodes.Where(i => i.InnerText == condition);
                foreach (HtmlNode node in nodlist)
                {
                    customindex.Add(doc.DocumentNode.ChildNodes.GetNodeIndex(node));
                    customindex.Add(customindex[customindex.Count-1] + 1);
                    customindex.Add(customindex[customindex.Count - 1] + 1);
                }
               
            }
            foreach(int ind in customindex)
            {
            foreach (var node in doc.DocumentNode.ChildNodes.Where(i=>doc.DocumentNode.ChildNodes.GetNodeIndex(i)==ind))
            {
               
                processTopNode(xamlString, node, true);
                if (node.Name == "br")
                {
                    writeBeginningTag(xamlString, tags["br"]);
                }
            }
            }
           // doc.DocumentNode.ChildNodes.RemoveAt(0);
         
            foreach (var node in doc.DocumentNode.ChildNodes.Where(i => !customindex.Contains(doc.DocumentNode.ChildNodes.GetNodeIndex(i))))
            {
               
                count++;

                if (count==1 || count==2|| count == doc.DocumentNode.ChildNodes.Where(i => !customindex.Contains(doc.DocumentNode.ChildNodes.GetNodeIndex(i))).Count() || count == doc.DocumentNode.ChildNodes.Where(i => !customindex.Contains(doc.DocumentNode.ChildNodes.GetNodeIndex(i))).Count() - 1 || count == doc.DocumentNode.ChildNodes.Where(i => !customindex.Contains(doc.DocumentNode.ChildNodes.GetNodeIndex(i))).Count() - 2)
                {
		...
		private static void processTopNode(StringBuilder xamlString, HtmlNode node, bool isTop = false)
		{
			...
			if (!string.IsNullOrWhiteSpace(node.InnerText))
			{
				...
				{
                    writeBeginningTag(xamlString, tags["div"]);
					nextNode = processNode(xamlString, node, true);
                    writeEndTag(xamlString, tags["div"]);
				}
			}
		...
		private static HtmlNode processNode(StringBuilder xamlString, HtmlNode node, bool isTop = false)
		{
			...
			else
			{
                if (node.NodeType == HtmlNodeType.Text)
                {
                    ...
                    {
                        xamlString.Append(node.InnerText.Trim().Substring(0, node.InnerText.Trim().Length - 1) + " " + node.InnerText.Trim().Substring(node.InnerText.Trim().Length - 1) + " ");
                      
                    }

                    else if (node.InnerText.Trim().StartsWith(":"))
                    {
                        xamlString.Append(" " + node.InnerText.Trim().Substring(0, 1) + " " + node.InnerText.Trim().Substring(1));
                    }
                    else
                    {
                        xamlString.Append(" " + node.InnerText.Trim());
                    }
